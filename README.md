...
## SAST local con Semgrep

Para demostrar el analisis de seguridad estatico en clase, el proyecto incluye un script listo para ejecutar:

```powershell
.\scripts\sast-semgrep.ps1
```

Ese script:

- analiza `src` y `test`
- usa reglas de Java y auditoria de seguridad
- guarda un reporte JSON y un resumen en `reportes/sast`
- permite repetir el analisis despues de corregir hallazgos

Si quiere ejecutar Semgrep manualmente, el comando base es:

```powershell
semgrep scan --config p/java --config p/security-audit src test
```

Para la evidencia de clase, conserve la captura de pantalla del primer analisis y luego el reporte generado por el script.
